...
const iconNavbarSidenav1 = document.getElementById('iconNavbarSidenav1');
const className1 = 'g-sidenav-lg-pinned';
const sideNavPinned = 'sideNavPinned';

if (iconNavbarSidenav1)
    iconNavbarSidenav1.addEventListener("click", toggleSidenav1)

function toggleSidenav1(e) {
    if (window.innerWidth >= 1200) {
        body.classList.toggle(className1);

        toggleNavHeaderIcon();

        if (body.classList.contains(className1))
            sessionStorage.setItem(sideNavPinned, 'true');
        else
            sessionStorage.setItem(sideNavPinned, 'false');
    } else
        sessionStorage.removeItem(sideNavPinned);
}

window.addEventListener("load", loadSideNavPref);

function loadSideNavPref() {
    if (window.innerWidth >= 1200) {
        let isExpand = sessionStorage.getItem(sideNavPinned) === "true"
        if (isExpand && !body.classList.contains(className1))
            body.classList.add(className1)
        else if (!isExpand && body.classList.contains(className1))
            body.classList.remove(className1)
...
function toggleNavHeaderIcon() {
    let icon = iconNavbarSidenav1.querySelector("i.fa-solid")
    if (icon) {
        if (body.classList.contains(className1)) {
...
function sideNavResize() {
    if (window.innerWidth < 1200)
        ...
        body.classList.remove(className)
        if (sessionStorage.getItem(sideNavPinned) === "true")
            body.classList.add(className1)
    }
}
...
/* Modal set focus and set cursor at the end of text*/
document.addEventListener('shown.bs.modal', function (event) {
    const elem = event.target;
    let input = elem.querySelector("#name");
    if (input) {
        input.focus();
...
document.querySelectorAll('.add-menu-form').forEach(form => {
    form.addEventListener("submit", onSubmitFormModal);
});
...
function onSubmitFormModal(event) {
    event.preventDefault();
    const elem = event.target;

    let isValid = true;
    let nameInput = elem.querySelector("#name");
    let nameError = elem.querySelector("#nameError");
...
function resetForm(formId) {
    ...
    if (form) {
        ...
        let nameError = form.querySelector('[id^="nameError"]');
        if (nameError) {
            nameError.classList.add("d-none");
            nameError.classList.remove("d-block");
        }
